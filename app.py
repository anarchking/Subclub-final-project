...
def upload():
    ...
    if request.method == "POST":
        ...
        if picture and allowed_types(picture.filename):
            ...
            # Variable for Path.
            new_path = "static" + app.config["UPLOAD_FOLDER"] + "/" + username

            # If no folder named "username" exists in uploads folder.
            if os.path.exists(new_path) == False:
                # Create username folder.
                os.mkdir(os.path.join("static", app.config["UPLOAD_FOLDER"], username))
            # If Image doesn't exist.
            if os.path.exists(new_path + "/" + filename) == False:
                # And if folder is empty.
                if os.listdir(new_path) == []:
                    # Save image as lower quality.
                    picture.save(os.path.join(new_path, filename))
